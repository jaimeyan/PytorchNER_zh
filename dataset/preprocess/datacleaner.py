...
class data_consistent:
    ...
    ###标点符号全部转换成英文
    def punctuation_translate(self, sent):
        sent_1 = sent.translate(self.table)
        return sent_1

    ###全角转半角
    def strQ2B(self, sent):
        ss = []
        for s in sent:
            rstring = ""
            for uchar in s:
...
if __name__ == '__main__':
    DC = data_consistent(r'./yidu-s4k/subtask1_training_part2.txt', r'./yidu-s4k/subtask1_training_part2_.txt',
                         charunification=False)
    DC.main_consistent()
